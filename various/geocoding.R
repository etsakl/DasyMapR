...
addresses = paste0(data$administrative_area,",",data$postal_code,",",data$route,", Greece")


#define a function that will process googles server responses for us.
getGeoDetails <- function(address){
  #use the gecode function to query google servers
  geo_reply = geocode(address, output=c("all"), messaging=TRUE, override_limit=TRUE,source = "google")
...
saveRDS(addresses, paste0(infile ,"_geocoded.rds"))
write.table(addresses, file=paste0(infile ,"_geocoded.csv"), sep=",", row.names=FALSE)
