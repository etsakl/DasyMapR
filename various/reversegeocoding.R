...
getGeoDetails <- function(location){
  #use the gecode function to query google servers
  geo_reply = revgeocode(location, output=c("all"), messaging=TRUE, override_limit=TRUE)
...
for (ii in seq(startindex, length(locations))){
  print(paste("Working on index", ii, "of", length(locations)))
  #query the google geocoder - this will pause here if we are over the limit.
  result = getGeoDetails(locations[ii,1:2])
  print(result$status)
  result$index <- ii
